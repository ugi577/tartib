...
import { formatOffset, formatTanggal, pesanError } from './template/bersama';
...
export function TemplateView() {
  ...
  return (
    <>
      ...
      <PanelEksporTemplate
        template={terpilih}
        jenisNama={jenis?.nama ?? 'SOP Acara'}
        memuatEditor={memuatEditor}
        onCetakPanduan={cetakPanduanA4}
      />
      ...
      {/* Dialog duplikat/versiBaru/arsip/hapus dari editor */}
      {dialog?.jenis === 'duplikat' && (
        <FormDialog
          terbuka
          judul={`Duplikat "${dialog.template.nama}"`}
      ...
      {/* Formulir panduan manual pengisian — hanya muncul di kertas (sesi 15).
          ...
          Sengaja DI LUAR pembungkus print:hidden di atas. */}
      {cetakPanduan && (
