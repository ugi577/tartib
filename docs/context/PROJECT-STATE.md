...
4. **Ahmed memverifikasi Batch V (impor)** — buka `?view=evaluasi`, pilih berkas .docx (mis. `SOP ACARA - Mahad Askar Quran.docx`), periksa pratinjau (fase/offset/item), simpan sebagai template, lalu buka hasilnya di tab Template (baca/duplikat/modifikasi). Gate V ditutup hanya oleh pernyataannya.
5. Setelah lulus: `./scripts/sync-publik.sh master --push` — snapshot publik berikutnya (saat ini repo publik masih `42429f3`, tanpa fitur impor Batch V).
