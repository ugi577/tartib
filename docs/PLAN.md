...
- **2026-08-22 — v1.20** — **Sesi 15 — header berlogo + template baku "SOP Baku (Panduan Manual)" + Cetak Panduan A4 (dua arahan Ahmed)**. Arahan: *(1) "masukkan ini 'Tartib — Pembuat SOP Acara' dalam header masukkan juga iconnya"*, *(2) "pada tab template buat template baku yg kosong atau di isi teks sebagai contoh, bisa didownload/export, dan jg bisa di print manual dan dan jadi akan panduan manual pengisian datanya"*. Empat commit: `ff89db1` logo Tartib (`LogoTartib.tsx` SVG inline squircle candy glass, artwork identik `icon.svg` favicon) + judul berikon di header + **rupa liquid glass** (WIP Ahmed di working tree — diverifikasi tsc/vitest 165/165 lalu dikomit; token tetap semantik di `tailwind.config.ts`), `0295761` `hapusTemplate` service (melengkapi tombol Hapus yang tanpa sengaja terbawa `a7a651f` — pelajaran: periksa diff per berkas sebelum `git add`), `8bd32e4` template baku + cetak: seed `TEMPLATE_PANDUAN` ("SOP Baku (Panduan Manual)", jenis Custom, **6 fase H-30…H+1, 17 item** semua diawali "Contoh:", idempoten **per nama** sehingga muncul juga di data yang sudah ada) + tombol **"Cetak Panduan (A4)"** di panel "Ekspor & Cetak Template" (pola Batch F: `flushSync` → `host.cetak({ jenis: 'panduanTemplate', templateId })` → `CetakPayload` bertambah varian — perluasan aditif K-04) → formulir kertas: kop + identitas acara + per fase kolom tanggal + per item divisi & kolom PIC; editor disembunyikan dari kertas saat mode cetak (`print:hidden` 12, `print:block` 4, `break-inside-avoid` 3). Verifikasi browser di 4173: template muncul setelah seed (balapan seed-vs-daftar pada muat pertama — muat ulang cukup), editor 6 fase/17 item + 3 tombol ekspor/cetak, klik Cetak memblokir webview (dialog cetak sistem terbuka — pola bukti Batch F; **cetak kertas fisik tetap verifikasi manual Ahmed**), Unduh .docx terpicu. tsc bersih, vitest **169/169** (+4 tes TEMPLATE_PANDUAN). **WIP Capacitor Ahmed (package.json/pnpm-lock/android/capacitor.config.ts/.gitignore) TIDAK dikomit — menunggu keputusannya.**
