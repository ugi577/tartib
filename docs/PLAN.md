...
- **2026-08-22 — v1.20** — **Sesi 15 — header berlogo + template baku "SOP Baku (Panduan Manual)" + Cetak Panduan A4 (dua arahan Ahmed) + header bilah penuh sesuai referensi gambar + perbaikan mobile.** Arahan: *(1) "masukkan ini 'Tartib — Pembuat SOP Acara' dalam header masukkan juga iconnya"*, *(2) "pada tab template buat template baku yg kosong atau di isi teks sebagai contoh, bisa didownload/export, dan jg bisa di print manual dan dan jadi akan panduan manual pengisian datanya"*, *(3) "buat header aplikasi sesuai referensi ini" + gambar*, *(4) perbaikan: "tampilan mobilenya malah kacau dan numpuk tabnya, dan bg header nya gak ada"*, *(5) perbaikan: "selalu perhatikan tampilannya untuk mobile, skrg tabnya sebagian tersembunyi"*. Tujuh commit: `ff89db1` logo Tartib (`LogoTartib.tsx` SVG inline squircle candy glass, artwork identik `icon.svg` favicon) + judul berikon + **rupa liquid glass** (WIP Ahmed diverifikasi lalu dikomit; token tetap semantik), `0295761` `hapusTemplate` service (melengkapi tombol Hapus yang tanpa sengaja terbawa `a7a651f` — pelajaran: periksa diff per berkas sebelum `git add`), `8bd32e4` template baku + cetak (seed `TEMPLATE_PANDUAN` idempoten per nama — 6 fase H-30…H+1, 17 item "Contoh:", jenis Custom — + tombol "Cetak Panduan (A4)", `CetakPayload` + varian `panduanTemplate`; formulir kertas kop/identitas/tanggal/PIC; penanda cetak 12/4/3), `01988d4` **header bilah penuh** sesuai referensi (sticky glass gradasi + `border-b border-white/70` + shadow, logo kiri, nav pil kanan), `0428b17` perbaikan mobile 1 (nav satu baris geser horizontal — ternyata menyembunyikan tab; latar header alpha 0.9; tagline tersembunyi < sm), `2abb36b` perbaikan mobile 2 — **nav mobile menjadi GRID 3×2: keenam tab SELALU terlihat penuh** (tanpa geseran/overflow). **ATURAN TETAP SESI 15: setiap perubahan UI wajib diverifikasi lebar ponsel (375/320px) — semua tab terlihat, nol overflow.** Verifikasi browser 4173: 375px & 320px — 6 tab, 2 baris rapi, `semuaTabTerlihat: true`, nol overflow dokumen; 768px — nav satu baris di baris logo + tagline tampil; template baku muncul, klik Cetak memblokir webview (**cetak fisik = verifikasi manual Ahmed**), Unduh .docx terpicu. tsc bersih, vitest **169/169**. **WIP Capacitor Ahmed TIDAK dikomit — menunggu keputusannya.**
