...
- **2026-08-22 — v1.22** — **Sesi 16 — "Buat e-Konfirmasi Kehadiran" (arahan Ahmed: *"buat menu di beranda 'Buat e-Konfirmasi kehadiran' dan tambahkan ini di file <title>e-Konfirmasi Kehadiran - Ma'had Askar Qur'an</title>, fungsi custom input sesuai kategori acara"* + acuan `Berkenan Konfirmasi Kehadiran.html`)** — `5388278` (4 berkas, +605 baris): (1) **Kartu baru di beranda** "Buat e-Konfirmasi Kehadiran" → view `?view=konfirmasi` (tanpa tab nav — hanya dari beranda); (2) **`lib/konfirmasi/eKonfirmasi.ts`** (murni, 7 tes): `susunHtmlKonfirmasi` menyusun SATU berkas HTML mandiri bergaya kartu emas (acuan berkas Ahmed: kertas `#FBF8F1`, emas `#A98436`, kop arab "معهد عسكر القرآن" + latin Mahad Askar, tag e-Konfirmasi, info acara Hari&Tanggal/Waktu/Tempat/Maps, form dasar nama/domisili/kategori kedatangan/WA/jumlah/catatan + bidang custom, tombol WA yang menyusun pesan `wa.me` dari SEMUA bidang, validasi wajib nama/kategori/jumlah, umpan "Konfirmasi Terkirim") dengan `<title>e-Konfirmasi Kehadiran - Ma'had Askar Qur'an</title>`; **`PRESET_KATEGORI`** — bidang custom per kategori acara (Tasyakuran Khatam: nama & jumlah santri khatam; Maulid: pembacaan & rombongan; Haflah: pentas; Wisuda: wisudawan & jumlah tamu; Dauroh: peserta & durasi; Rapat Wali Santri: wali & ananda; PHBI: rombongan & jumlah; Custom: tanpa preset); (3) **`EKonfirmasiView`**: pilih acara (prefill judul+tanggal, deteksi kategori dari jenis acara), tombol "Isi sesuai kategori", editor bidang (label/tipe teks-pilihan/opsi koma/hapus/tambah), pratinjau iframe langsung, unduh `e-Konfirmasi-<nama>.html`; keamanan: JSON di `<script>` di-escape `\u003c/\u003e/\u0026` (anti-breakout `</script>`), HTML di-escape. Verifikasi browser dev 3000 (data Ahmed): pilih acara "Khatam Tasmi 21 Agustus" → judul/tanggal terisi + kategori Tasyakuran Khatam → "Isi sesuai kategori" memunculkan 2 bidang preset → iframe menampilkan kartu (tag e-Konfirmasi, h1 acara, tombol WA, input nama) → unduh HTML terpicu; 375px nol overflow, tab utuh. Fix selama verifikasi: query `orderBy('tanggal')` gagal (tanggal tak ber-index di skema) → sortir di JS. tsc bersih, vitest **202/202** (25 berkas).
