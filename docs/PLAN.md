...
- **2026-08-29 — v1.30** — **Sesi 21 — review format HP + APK debug berisi seluruh fitur** (dua arahan Ahmed: *"review lg format hp nya"* lalu *"commit semua dan build apk debug full semua fitur baru"*). Dua bagian: (1) **review format HP** — audit ulang ke-7 view (beranda, template, acara, sop, tamu, evaluasi, pengaturan) di 375px/360px/320px via browser in-app: **nol overflow horizontal** di semua viewport; header 214px @375px; @320px nav membungkus 2 baris (129px) dan tombol aksi kartu SOP turun baris sendiri (pembungkusan responsif disengaja, semua teks utuh); temuan awal header ~6000px ternyata **dev server menyajikan CSS 404** (`/_next/static/css/...` — server lama mati, halaman tanpa styling, SVG jatuh ke ukuran intrinsik 359px, PitaIslami menumpuk ~12 baris) — **bukan bug aplikasi**: dev di-restart → CSS HTTP 200 (39,5 KB) → semua view pulih; (2) **APK debug penuh** — gate: tsc bersih, lint bersih, vitest **245/245** (28 berkas), `pnpm build` statis sukses (5 halaman prerender); `npx cap sync android` — web assets `out/` → `android/app/src/main/assets/public`, 2 plugin: `@capacitor/filesystem@8.1.3`, `@capacitor/share@8.0.1`; `./gradlew assembleDebug` **BUILD SUCCESSFUL** → `android/app/build/outputs/apk/debug/app-debug.apk` **4,8 MB** (5.034.610 byte); isi diverifikasi `unzip -l`: `index.html` + `icon.svg` + `404.html` + 18 berkas `_next/static` dengan build ID `NhYMWysBvIynxVE1HBjBq` — cocok dengan `pnpm build` sesi ini → APK benar-benar berisi fitur sampai sesi 20 (K-25). Working tree bersih di awal sesi (semua sudah ter-commit) → nol perubahan kode; komit sesi ini = dokumen. **Tersisa: verifikasi manual Ahmed — pasang APK di device (seluruh fitur + cadangan/ekspor .docx), cetak fisik (dialog cetak sistem tidak bisa diuji browser otomatis), dan sinkron publik.**
- **2026-08-29 — v1.29** — **Sesi 20 — rupa kaca krem–teal–emas dari referensi UI Ahmed (K-25)** — arahan: *"terapkan ui ini ke proyek, cek dan pahami dl, tetap pertahankan ornamen yg sudah ada, jangan terapkan garis gold yg gelombang dan bintang 4 arah dr file ini"* (berkas `~/Documents/ui tartib.html`). Tema emerald/sky diganti **teal gelap + emas di atas krem**: `tailwind.config.ts` — `aksen` emerald→**teal** (600 `#246b5a`/700 `#1a5a4a`/800 `#144738`), palet **`emas` baru** (500 `#c5a87b`), `permukaan.dasar` **`#f5f5f0`** (krem), `glowAksen` = kilau putih 1px + bayangan teal 50%; `kelas.ts` — kartu/kartuIsi putih translusen + `border-emas-300/60` + blur, `tombolUtama` gradasi `aksen-700→600` + `ring-emas-400/70` + glow, `badgeInfo` emas (`emas-100/70` + teks `emas-700`); `page.tsx` — header gradasi `aksen-800→700→600` + `border-b border-emas-400/40` + judul putih, tab pil aktif = gradasi teal gelap + `ring-emas-400/80` + glow, baris tab kaca gradasi aksen→putih→emas; `globals.css` — body `#f5f5f0` + radial-gradient teal/emas, fokus ring aksen. **Ornamen lama dipertahankan** (BintangDelapan ×3 + LogoTartib + PitaIslami 12 bintang, diwarnai samar emas); **garis gold gelombang & bintang 4 arah dari berkas referensi TIDAK diterapkan** sesuai arahan (tidak pernah masuk kode). `LogoTartib`/`icon.svg` kepala diwarnai teal (`#518e7f→#246b5a`), badge Tamu "SELESAI" → token aksen. Cetak tetap `@page A4 14mm` + `background-image: none` (glow kaca tidak tercetak). tsc & lint bersih, vitest **245/245** (28 berkas), build statis sukses; verifikasi browser 375px + 1280×900 via computed style (body krem `rgb(245,245,240)`, header gradasi teal + border emas, tab aktif `rgb(26,90,74)→rgb(36,107,90)` + ring emas + glow, kartu border emas radius 20px, tombol utama gradasi teal + ring emas, badge info emas, beranda 2 kolom @1280, nol overflow, 16 SVG ornamen utuh) — menunggu kesan visual Ahmed di device.
...
- **2026-08-22 — v1.24** — **Sesi 16 — unduhan Android PASTI (plugin Filesystem+Share) + ikon launcher baru (arahan Ahmed: *"belum bisa di unduh htmlnya d hp, dan ganti lg icon nya di hp pakai ini"* + gambar)** — `b382951` + `eb4229c`/`b91385b`: (1) **Unduhan native** — Web Share API browser terbukti belum cukup di perangkat; solusi pasti: plugin **`@capacitor/filesystem` + `@capacitor/share`** — `src/app/unduhNative.ts` (di src/app agar Gate A src/tartib tetap bebas impor luar) menulis berkas ke cache aplikasi lalu membuka **lembar berbagi Android via Share+FileProvider**; dipasang di titik masuk `page.tsx` lewat `aturBagikanNative` (setter di `lib/unduh.ts`, tartib tetap murni); `lib/unduh.ts` berlapis: native → Web Share API → anchor. (2) **Ikon launcher HP diganti** dari gambar Ahmed (1024×1024): script PIL — legacy `ic_launcher.png`/`ic_launcher_round.png` 48–192 px per densitas, adaptive foreground 108–432 px (konten ~62% zona aman di kanvas transparan), warna latar adaptive = rata-rata tepi gambar (`#1c6759`). APK debug dibangun ulang: plugin terdaftar di `capacitor.plugins.json` + gradle, ikon 192px terverifikasi di dalam APK, 7,8 MB. tsc bersih, vitest **203/203**. **Uji di HP = verifikasi manual Ahmed** (lembar berbagi muncul; ikon baru di launcher — mungkin perlu uninstall/install ulang agar cache ikon segar).

- **2026-08-22 — v1.23** — **Sesi 16 — unduhan lintas platform (bug Ahmed: *"unduh html gak bisa di android, di browser bisa"*)** — `9dd538a` (5 berkas): penyebab — WebView Android **mengabaikan anchor `download`** (tidak ada pengunduh bawaan), makanya unduhan hanya jalan di browser desktop. Perbaikan — `lib/unduh.ts` (`unduhBerkas(nama, blob)`): coba **Web Share API** dulu (`navigator.canShare({files})` → `navigator.share` membuka lembar berbagi Android: Simpan ke Drive/WhatsApp/dll.; AbortError = batal, share gagal lain → fallback), lalu **fallback anchor** untuk desktop. Dipakai di SEMUA unduhan aplikasi: HTML e-Konfirmasi, .docx template, Markdown buku acara, cadangan .json (pesan sukses disesuaikan "dibagikan"/"diunduh"). Verifikasi browser: unduh HTML & .docx tetap terpicu (fallback), pesan tampil; tsc bersih, vitest **203/203**. **Uji Android = verifikasi manual Ahmed** (lembar berbagi muncul; Web Share butuh konteks aman — origin Capacitor Android `https://localhost` mendukung).

- **2026-08-22 — v1.22** — **Sesi 16 — "Buat e-Konfirmasi Kehadiran" (arahan Ahmed: *"buat menu di beranda 'Buat e-Konfirmasi kehadiran' dan tambahkan ini di file <title>e-Konfirmasi Kehadiran - Ma'had Askar Qur'an</title>, fungsi custom input sesuai kategori acara"* + acuan `Berkenan Konfirmasi Kehadiran.html`)** — `5388278` + **`94acd44`** (generator sepenuhnya manual: dropdown "Pilih acara" dihapus, semua isian kosong & manual, kategori dipilih manual dari `JENIS_ACARA_BAKU`) + **`a3c1c17` arahan lanjut Ahmed (*"…pastikan kop/header ini bisa di ganti juga"* + cuplikan kop)** — **kop/header kini bisa diganti**: `DataKonfirmasi` + `kopArab`/`kopLatin`/`tag` (kosong = bawaan `KOP_ARAB_BAKU` "مَعْهَدُ عَسْكَرِ الْقُرْآنِ", `KOP_LATIN_BAKU` "Mahad Askar Cijulang • Cansebu", `TAG_BAKU` "e-Konfirmasi Kehadiran"), tiga isian baru di kartu "Acara & kop" generator. Isi lain: (1) **Kartu baru di beranda** "Buat e-Konfirmasi Kehadiran" → view `?view=konfirmasi` (tanpa tab nav); (2) **`lib/konfirmasi/eKonfirmasi.ts`** (murni, 8 tes): `susunHtmlKonfirmasi` menyusun SATU berkas HTML mandiri bergaya kartu emas (acuan berkas Ahmed: kertas `#FBF8F1`, emas `#A98436`, kop arab + latin, tag, info acara Hari&Tanggal/Waktu/Tempat/Maps, form dasar nama/domisili/kategori kedatangan/WA/jumlah/catatan + bidang custom, tombol WA menyusun pesan `wa.me` dari SEMUA bidang, validasi wajib nama/kategori/jumlah, umpan "Konfirmasi Terkirim") dengan `<title>e-Konfirmasi Kehadiran - Ma'had Askar Qur'an</title>`; **`PRESET_KATEGORI`** — bidang custom per kategori (Tasyakuran Khatam, Maulid, Haflah, Wisuda, Dauroh, Rapat Wali Santri, PHBI; Custom tanpa preset); (3) **`EKonfirmasiView`**: tombol "Isi sesuai kategori", editor bidang (label/tipe teks-pilihan/opsi koma/hapus/tambah), pratinjau iframe langsung, unduh `e-Konfirmasi-<nama>.html`; keamanan: JSON di `<script>` di-escape `\u003c/\u003e/\u0026` (anti-breakout `</script>`), HTML di-escape. Verifikasi browser dev 3000: field manual kosong, kategori manual → preset 2 bidang, **kop diganti (Arab/Latin/tag baru) terbukti masuk ke srcdoc pratinjau, teks lama hilang** (render & escaping juga diuji 8 tes unit), unduh terpicu; 375px nol overflow. tsc bersih, vitest **203/203** (25 berkas).

- **2026-08-22 — v1.21** — **Sesi 16 — identitas header baru (gradasi + judul + ikon kalender-ceklis) + TAB PENGATURAN baru (Tentang pindah ke dalamnya) + cadangan/pemulihan data .json.** Arahan Ahmed: *(1) "ganti headernya dengan gradasi warna, tulisan, icon seperti ini" + gambar*, *(2) "buat tab baru pengaturan, dan tambahkan seting dari saran terbaik kamu, termasuk juga masukkan tab 'tentang' yg ada ke dalam tab pengaturan ini, dan rapikan kembali isinya sesuai data yang ada"*, *(3) "tulis kembali 'Tartib — Pembuat SOP Acara', dan tag di bawahnya hapus kata tartib"*, *(4) "icon LogoTartib.tsx ini tdk pas yg lebih pas, isyarat acara dan kotak centang cek list"*, *(5) "push semua"*. Keputusan tercatat di **K-21**. Isi: **(a) Header** — gradasi dipertegas `sky-200 → sky-50 → emerald-200`, judul tebal `text-lg`/`sm:text-2xl` (teks tetap "Tartib — Pembuat SOP Acara"; "Tartib —" dibuang dari tagline), ikon di atas cahaya putih; **`LogoTartib` digambar ulang dua kali** — iterasi pertama dokumen+panah-grafik mengikuti gambar referensi, lalu **dibuang atas koreksi Ahmed** dan diganti **kalender bergantung dua dengan tiga baris ceklis (dua tercentang, satu kosong)**; favicon `src/app/icon.svg` disamakan di kedua iterasi. **(b) Tab Pengaturan** (`src/tartib/components/PengaturanView.tsx`, sub-navigasi Umum / Data & Cadangan / Tentang): *Umum* — identitas & kop cetak (nama lembaga + baris alamat, pratinjau) dan nilai baku kalkulator porsi (cadangan, buffer %, tim pencuci) dengan contoh hidup; *Data & Cadangan* — statistik 12 tabel, **unduh cadangan .json + pulihkan dari berkas** (`cadanganService`), muat ulang data contoh (seed), hapus semua data, serta client ID + status sesi Google Drive yang dipindah dari panel ekspor template; *Tentang* — isi tab lama, dirapikan agar cocok dengan fitur yang benar-benar ada (impor .docx, ekspor .docx/Drive, cetak panduan A4, cadangan) dan angka divisi baku dibaca dari `DIVISI_BAKU`. **Tab "Tentang" dihapus dari navigasi utama** (tetap enam tab); `?view=tentang` dipetakan ke Pengaturan bagian Tentang. **(c) Kop cetak** `KopCetak` (`print:block`) disisipkan ke empat blok cetak (buku acara, laporan eksekusi, lembar tugas, panduan template); kosong = tanpa kop seperti sebelumnya. **(d) Nilai baku porsi** kini mengalir ke kalkulator `TamuView` lewat hook `usePengaturan` + event `tartib:pengaturan`. Berkas baru: `lib/pengaturan.ts` (+11 tes), `lib/usePengaturan.ts`, `services/cadanganService.ts` (+10 tes), `components/PengaturanView.tsx`, `components/KopCetak.tsx`. **Verifikasi browser 3000 (dev Ahmed):** simpan pengaturan → localStorage persis seperti diketik; contoh porsi hidup benar (100 hadir @130% + 50 + 20 + cadangan 20 = 220 porsi, peralatan 132); **cadangan diuji sungguhan** — unduh (83 baris), pulihkan berkas 83 baris lewat dialog konfirmasi, berkas asing ditolak ("Berkas ini bukan cadangan Tartib") tanpa menyentuh data; **alur porsi diuji lewat acara uji yang dibuat lalu dihapus kembali** (dipulihkan dari cadangan — DB kembali 83 baris / 0 acara), pengaturan uji dikembalikan ke nilai baku; `?view=tentang` membuka Pengaturan bagian Tentang dengan tab tersorot; **375px nol overflow di ketiga bagian** (sub-navigasi sempat mepet → diganti pil `flex-wrap`); konsol nol error. tsc bersih, vitest **190/190** (24 berkas). **Cetak fisik kop = verifikasi manual Ahmed** (klik Cetak memblokir webview, seperti sesi sebelumnya). **(e) WIP Capacitor Ahmed AKHIRNYA DIKOMIT** atas perintah "push semua" — 53 berkas proyek Android (~780 KB, hasil build dikecualikan `.gitignore` buatan Ahmed) + `capacitor.config.ts` + dependensi `@capacitor/*` di `package.json`/lockfile. **Tambahan sesi 16 (arahan Ahmed: *"tambahkan watermark dev achshoks@askarquran di bawah seluruh halaman yg terbuka dan kontak 085212351117 di tentang kontak dev app ini"*):** (f) **Watermark** — `<footer>` tetap `fixed inset-x-0 bottom-0` bertuliskan `dev achshoks@askarquran` (kecil, tengah, redup), `pointer-events-none` (tak menghalangi sentuhan) + `print:hidden` (tak ikut tercetak), tampil di SEMUA view (diverifikasi beranda & template; posisi bawah di 375px, nol overflow); (g) **Kontak Developer** — kartu baru di bagian Tentang (`TentangView` dalam tab Pengaturan): `achshoks@askarquran` + tel/WhatsApp **085212351117** (tautan `tel:`); (h) **APK debug dibangun** (`android/app/build/outputs/apk/debug/app-debug.apk`, 4,7 MB — `pnpm build` → `cap sync android` → `gradlew assembleDebug`, web assets `out/` tersalin; Java 21 + SDK `~/Library/Android/sdk`); (i) commit penutup: `7fbfe5b` (kode sesi 16 + watermark + kontak — WIP Ahmed di working tree yang ikut diverifikasi tsc/vitest 195/195), `eab1b7f` (android/ + capacitor.config.ts — melengkapi klaim "push semua" yang sebelumnya baru tercatat di dokumen, berkasnya masih untracked).

- **2026-08-22 — v1.20** — **Sesi 15 — header berlogo + template baku "SOP Baku (Panduan Manual)" + Cetak Panduan A4 (dua arahan Ahmed) + header bilah penuh sesuai referensi gambar + perbaikan mobile + ornamen islami & pemisahan header-tab.** Arahan: *(1) "masukkan ini 'Tartib — Pembuat SOP Acara' dalam header masukkan juga iconnya"*, *(2) "pada tab template buat template baku yg kosong atau di isi teks sebagai contoh, bisa didownload/export, dan jg bisa di print manual dan dan jadi akan panduan manual pengisian datanya"*, *(3) "buat header aplikasi sesuai referensi ini" + gambar*, *(4) "tampilan mobilenya malah kacau dan numpuk tabnya, dan bg header nya gak ada"*, *(5) "selalu perhatikan tampilannya untuk mobile, skrg tabnya sebagian tersembunyi"*, *(6) "berikan ornamen islami pada header dan pisahkan tempat header dan tab2 dibawahnya"*. Delapan commit: `ff89db1` logo Tartib (`LogoTartib.tsx` SVG inline squircle candy glass, artwork identik `icon.svg` favicon) + judul berikon + **rupa liquid glass** (WIP Ahmed diverifikasi lalu dikomit; token tetap semantik), `0295761` `hapusTemplate` service (melengkapi tombol Hapus yang tanpa sengaja terbawa `a7a651f` — pelajaran: periksa diff per berkas sebelum `git add`), `8bd32e4` template baku + cetak (seed `TEMPLATE_PANDUAN` idempoten per nama — 6 fase H-30…H+1, 17 item "Contoh:", jenis Custom — + tombol "Cetak Panduan (A4)", `CetakPayload` + varian `panduanTemplate`; formulir kertas kop/identitas/tanggal/PIC; penanda cetak 12/4/3), `01988d4` header bilah penuh sesuai referensi, `0428b17` perbaikan mobile 1 (latar alpha 0.9, tagline < sm), `2abb36b` perbaikan mobile 2 (**nav grid 3×2 — semua tab selalu terlihat; ATURAN TETAP: setiap perubahan UI wajib diverifikasi 375/320px**), `0c5db78` **ornamen islami + pemisahan header** — `src/tartib/components/OrnamenIslami.tsx` (`BintangDelapan` khatam dua persegi 45°, `PitaIslami` deretan bintang): tiga bintang samar di latar area identitas + pita pemisah; **baris tab kini area TERPISAH di bawah header** (latar putih kaca sendiri, `border-t`, tab tengah di desktop, grid 3×2 di ponsel — selisih 35px dari judul, terukur di browser). Verifikasi browser 4173: 375px — 6 tab terlihat, ornamen 15 SVG, nav terpisah di bawah judul, nol overflow; 768px — tab satu baris tengah; template baku muncul, klik Cetak memblokir webview (**cetak fisik = verifikasi manual Ahmed**), Unduh .docx terpicu. **`910cd7b` perbaikan sesi 15 lanjutan** (laporan Ahmed: *"tombol import sop dr dokumen gak keliatan tulisannya, dan kebesaran ph nya"*): tombol "Pilih Berkas .docx" yang semula `::file-selector-button` bawaan browser (teks tak tampil & ukuran membengkak di WebView) diganti **tombol aplikasi** (`KELAS.tombolUtama` + input `sr-only` dipicu label, + reset value agar berkas sama bisa dipilih ulang); sekalian perbaiki overflow 375px pada baris aksi kartu template (5 tombol tanpa wrap → `flex-wrap`; penyebab: tombol Hapus). **`c58b50e` arahan lanjut Ahmed** (*"jangan batasi pada docx sj, tulis 'Import SOP Dokumen' dan 'pilih file' sj"*): judul panel → **"Import SOP Dokumen"**, tombol → **"Pilih File"**, atribut `accept` dihapus (semua jenis berkas bisa dipilih; berkas bukan .docx tetap ditolak parser dengan pesan jelas). **`41f0af4` tindak lanjut laporan Ahmed** (*"kenapa saat import sop acara ini masih ada tanda kuning ini?"* — tanda kuning = emoji ⚠️ yang TERTULIS di heading dokumen sumber, terverifikasi byte-nya di `document.xml`): `buangSimbolPeringatan` di parser — label fase & judul item **otomatis dibuang ⚠/⚠️** saat impor (spasi di sekitarnya dipadatkan); 2 tes ditambah/diubah (label & item beremoji). **`9fee131` uji bolak-balik impor↔ekspor atas arahan Ahmed** (*"uji bolak balik, import exportnya dan pastikan tdk ada bagian yg tidak di export - import"*): temuan — ekspor lama hanya menulis nama/jenis/fase/judul, jadi **divisi, catatan, wajib, rumusQty, urutan, versi HILANG** di round-trip; perbaikan — `tulisDocx` menyisipkan entry **`tartib/template.json`** (data LENGKAP: nama, jenisAcara, catatan, versi, urutan fase, per item divisi/catatan/wajib/rumusQty) di dalam .docx (tetap sah untuk Word — entry tambahan diabaikan), baris item di document.xml ikut memuat catatan & divisi; `sopDariJson` dibaca importer lebih dulu (berkas ekspor Tartib = data penuh; heuristik document.xml hanya untuk berkas biasa); UI: `pilihBerkasImpor` cek JSON + preset jenis acara, `simpanImpor` memetakan catatan/wajib/rumusQty, `dataDocxUntukEkspor` mengirim semua field. **Bukti e2e dokumen asli (test sementara, dihapus): impor 9 fase/57 item → ekspor → impor ulang → SEMUA field cocok, nol item hilang**; tes tetap: 4 entry ZIP + round-trip JSON lossless + 4 tes `sopDariJson`; tsc bersih, vitest **195/195** (bertambah karena tes WIP Ahmed: pengaturan, cadangan — ikut lulus). Verifikasi browser: unduh .docx terpicu. **WIP Capacitor Ahmed TIDAK dikomit — menunggu keputusannya.**
